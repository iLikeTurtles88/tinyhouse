  ...
  return (
    <div className={`flex flex-col min-h-screen ${spaceGrotesk.className}`}>
      ...
      <section className="relative h-96 flex items-center justify-center text-center text-white bg-gray-800 overflow-hidden">
        <Image
          src="/images/entete.jpg" // <-- Chemin vers ton image locale
          alt="En-tête Tiny House" // N'hésite pas à ajuster le texte alternatif
          fill // Remplace layout="fill"
          style={{ objectFit: "cover" }} // Remplace objectFit="cover"
          className="absolute z-0 opacity-60"
          priority // Ajoute ceci car c'est une image importante (LCP)
        />
      ...
      <section className="bg-secondary p-6 py-12">
        ...
        <p className="text-md text-muted-foreground text-center mb-6">
          Intéressé par une de nos tiny houses ? Contactez-nous pour plus
          d'informations.
        </p>
        <ContactForm />
      </section>
